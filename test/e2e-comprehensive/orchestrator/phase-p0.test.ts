...
import { serverEvents, cacheAdmin, fileProxyEvents, ptyEvents } from "./server-events.js";
...
test("A1-bash-basic: model 触发 Bash → server 中转 client 执行 → tool_result 回写", async () => {
  ...
  const toolResult = cap[1].toolResultsCurrentTurn[0];
...
test("B4-ancestor-claudemd: ancestor 段 bootstrap 不在 set -u 下崩 + ancestor CLAUDE.md 可读", async () => {
  ...
  const cap = await mockAdmin.captured();
  const toolResult = cap.at(-1)?.toolResultsCurrentTurn[0];
...
test("A2-fs-rwe: write → read → edit 三件套对临时文件", async () => {
  ...
  // I4 后：cap[N].toolResultsCurrentTurn 只含本轮 tool_result；多 tool 取 [i]，
  // 单 tool 取 [0]。旧 .toolResults.at(-1) 解法已弃用。
  // turn 2 携带 turn 1 的 write 结果
  const writeResult = cap[1].toolResultsCurrentTurn[0];
  ...
  const readResult = cap[2].toolResultsCurrentTurn[0];
  ...
  const editResult = cap[3].toolResultsCurrentTurn[0];
...
test("A3-search: glob + grep 在多文件 fixture 内", async () => {
  ...
  const globResult = cap[1].toolResultsCurrentTurn[0];
  ...
  const grepResult = cap[2].toolResultsCurrentTurn[0];
...
test("A4-shadow-mcp: dual-path 不变量（mcp 路径 false / 内置 Bash deny true）", async () => {
  ...
  const shadowResult = cap[1].toolResultsCurrentTurn[0];
  ...
  const denyResult = cap[2].toolResultsCurrentTurn[0];
...
// ============================================================
// B1-home-claude-snapshot: server 启动期 ~/.claude/ 经 home-claude FUSE root 链路。
// 主断言: file-proxy.read.served event 出现 root="home-claude" + 目标 relPath。
//   - mcp__cerelay__read 是 client-routed → 跑在 client 端原文，**绕过 server FUSE
//     链路**，不能作为主断言（这是 P0-B 终审 Codex 抓到的根因，详 §11.1 C1）。
//   - server 端 emit "file-proxy.read.served" 在 cache snapshot 灌入 daemon 时触发；
//     即使 daemon perm cache 命中、不再回 server 拿 read，server 也已经"知道"并
//     "提供"了这个 file 的内容——这是 server FUSE 链路真触发的最直接证据。
// 次断言: 保留 mcp__cerelay__read 验 client 端原文 → 双证据互相印证。
// ============================================================
test("B1-home-claude-snapshot: server FUSE 链路服务 ~/.claude/<file>（admin event 主证据）", async () => {
  ...
  const baselineEventId = (await serverEvents.fetch({})).at(-1)?.id ?? 0;
  ...
  // 主断言：server FUSE 链路真把 home-claude 的目标 relPath 喂给了 daemon snapshot。
  // 注意：home-claude root 指向 ~/.claude/，event detail.relPath 是 ROOT 内的相对路径
  // （不含 ".claude/" 前缀），所以 ~/.claude/case-b1-marker.md → relPath="case-b1-marker.md"。
  const homeClaudeRel = homeRel.replace(/^\.claude\//, "");
  await fileProxyEvents.waitForReadServed({
    root: "home-claude",
    relPath: homeClaudeRel,
    since: baselineEventId,
    timeoutMs: 10_000,
  });

  // 次断言（client 路径）：mcp__cerelay__read 也能取到 marker 原文
  const cap = await mockAdmin.captured();
  assert.equal(cap.length, 2, `expected 2 turns, got ${cap.length}`);
  const readResult = cap[1].toolResultsCurrentTurn[0];
  assert.ok(readResult, "expected read tool_result on turn 2");
  assert.equal(readResult.is_error, false, "read should succeed");
  assert.match(
    readResult.content,
    new RegExp(marker),
    "client-routed read should also reflect marker (旁证)"
  );
...
// ============================================================
// B2-claude-json-read: server 读 ~/.claude.json 经 home-claude-json FUSE root 链路。
// 主断言：file-proxy.read.served event 出现 root="home-claude-json" + relPath=""。
//   home-claude-json 是单文件 root（绑定 ~/.claude.json 自身），cache scope=claude-json
//   的 relPath 在 cache manifest 里是空串（参考 file-agent/scope-adapter.ts）。
// 次断言：mcp__cerelay__read 取到 marker 原文。
// CC 启动时会 parse .claude.json，所以 fixture 必须是合法 JSON。
// ============================================================
test("B2-claude-json-read: server FUSE 链路服务 ~/.claude.json（admin event 主证据）", async () => {
  ...
  // 主断言：server FUSE 链路真把 home-claude-json 喂给 daemon snapshot。
  // home-claude-json 是单文件 root，cache scope=claude-json 内的 relPath 是空串。
  await fileProxyEvents.waitForReadServed({
  ...
  // 次断言：client 路径也能取到 marker 原文
  const cap = await mockAdmin.captured();
  assert.equal(cap.length, 2, `expected 2 turns, got ${cap.length}`);
  const readResult = cap[1].toolResultsCurrentTurn[0];
  assert.ok(readResult, "expected read tool_result on turn 2");
  assert.equal(readResult.is_error, false, "read should succeed");
  assert.match(readResult.content, new RegExp(marker), "client-routed read should reflect marker (旁证)");
});

// ============================================================
// B3-project-claude: server 读 {cwd}/.claude/<file> 经 project-claude bind mount。
// 主断言：file-proxy.read.served event 出现 root="project-claude" + relPath="marker-b3.txt"。
//   project-claude 不进 cache（项目级文件），启动期 server 通过 client snapshot
//   round-trip 拉到 entries → 灌入 daemon snapshot → emit servedFrom="snapshot-client"。
// 次断言：mcp__cerelay__read 取到 marker 原文。
// fixture 在 cwd/.claude/marker-b3.txt 放 marker，避开 hook 注入的 settings.local.json。
// ============================================================
test("B3-project-claude: server FUSE 链路服务 {cwd}/.claude/<file>（admin event 主证据）", async () => {
  ...
  // 主断言：server FUSE 链路真把 project-claude 的 marker-b3.txt 喂给 daemon snapshot。
  await fileProxyEvents.waitForReadServed({
    root: "project-claude",
    relPath: "marker-b3.txt",
    since: baselineEventId,
    timeoutMs: 10_000,
  });
  ...
  assert.match(
    readResult.content,
    new RegExp(marker),
    "client-routed read should reflect marker (旁证)"
  );
...
// ============================================================
// C2-revision-ack: sync 完成后 server manifest revision >= client 已 ack 的 revision
// 且 drift ≤ 50（runtime FUSE 访问 / TTL 续期会持续 bump）
// 守护 revision 单调、ack 配对、bound drift；防 batch 丢失或 server 漏 ack 跳号
// ============================================================
test("C2-revision-ack: server revision >= client acked + drift <= 50", async () => {
...
// ============================================================
// D1-cwd-aligned: server namespace 内 cwd === client 上报 cwd。
// 主断言：pty.spawn.ready admin event 的 detail.cwd === client 上报 cwd。
//   POSIX spawn 契约：child 启动时 process.cwd() === parent 配置的 cwd（pty-session.ts
//   spawn 时把 this.cwd 设为 child cwd），等价于 namespace 内 pwd 输出。
//   Codex 终审 §11.1 C1 要求："不能用 mcp__cerelay__bash 跑在 client 端绕过 server"。
// 次断言：保留 mcp__cerelay__bash 跑 pwd 验 client 端也对齐（双证据）。
// ============================================================
test("D1-cwd-aligned: server namespace cwd === client 上报 cwd（admin event 主证据）", async () => {
  ...
  // 主断言：server 端 PTY child 的 cwd === client 上报 cwd（POSIX spawn 契约）
  const spawnEvent = await ptyEvents.waitForSpawnReady({
    expectedCwd: cwd,
    since: baselineEventId,
    timeoutMs: 30_000,
  });
  assert.equal(spawnEvent.detail.cwd, cwd, `pty.spawn.ready.cwd 应该 === client 上报 cwd=${cwd}`);

  // 次断言：client 端 pwd 也对齐（旁证）
  const cap = await mockAdmin.captured();
  assert.equal(cap.length, 2);
  const pwdResult = cap[1].toolResultsCurrentTurn[0];
  assert.ok(pwdResult, "expected pwd tool_result");
  assert.match(
    pwdResult.content,
    new RegExp(`^[^]*${cwd.replace(/\//g, "\\/")}[^]*$`),
    `client-routed pwd should also include cwd=${cwd} (旁证), got: ${pwdResult.content}`,
  );
...
// ============================================================
// D2-home-aligned: server namespace 内 $HOME === client 上报 home。
// 主断言：pty.spawn.ready admin event 的 detail.homeDir === client 上报 home。
//   POSIX 契约：child 的 process.env.HOME = parent 配置的 env.HOME，等价于
//   namespace 内 echo $HOME 输出。
// 次断言：保留 mcp__cerelay__bash echo $HOME 验 client 端对齐。
// ============================================================
test("D2-home-aligned: server namespace $HOME === client 上报 home（admin event 主证据）", async () => {
  ...
  // 主断言：server 端 PTY child 的 homeDir === expectedHome
  const spawnEvent = await ptyEvents.waitForSpawnReady({
    expectedCwd: cwd,
    since: baselineEventId,
    timeoutMs: 30_000,
  ...
  assert.equal(
    spawnEvent.detail.homeDir,
    expectedHome,
    `pty.spawn.ready.homeDir 应该 === ${expectedHome}（namespace 内 $HOME 实际值）`,
  );

  // 次断言：client 端 $HOME 也对齐（旁证）
  const cap = await mockAdmin.captured();
  const homeResult = cap[1].toolResultsCurrentTurn[0];
  assert.ok(homeResult, "expected echo $HOME tool_result");
  assert.match(
    homeResult.content,
    new RegExp(expectedHome.replace(/\//g, "\\/")),
    `client-routed $HOME should also be ${expectedHome} (旁证), got: ${homeResult.content}`,
  );
...
// ============================================================
// E1-snapshot-redact: ~/.claude/settings.json 含 ANTHROPIC_API_KEY 时，server
// 启动期 snapshot 灌入 daemon 时**必须**经过 redact，event detail.site === "snapshot"。
//   E2E 可稳定触发 snapshot site：homeFixture 写入 settings.json → cache 同步 →
//   server 启动期 buildSnapshotFromManifest 命中 → redactSettingsAndReport("snapshot",...)。
//   cache-hit 与 passthrough 两个 site 不可在 e2e 稳定触发（依赖 daemon perm cache
//   失效与 cache miss 时机），降级到 server 单测 server/test/file-proxy-redact-site.test.ts
//   覆盖。详见 docs §11.1 C2。
// ============================================================
test("E1-snapshot-redact: settings.json 经 server snapshot 出口被 redact (site=snapshot)", async () => {
  const caseId = "case-e1-snapshot";
  await writeFixture(caseId, { ".keep": "" });
  const cwd = clientCwd(caseId);
  ...
  await mockAdmin.loadScript({
    name: "p0-e1-snap-turn1",
    match: { turnIndex: 1 },
    respond: scriptText("e1 snapshot site ok"),
  });
  ...
  const result = await clients.run("client-a", {
    prompt: "trigger settings redact snapshot [E1-SNAP-MARKER]",
    cwd,
    homeFixture: {
  ...
  // 主断言：必须出现 site=snapshot 的 redact event（不接受其他 site 顶替）
  const newEvents = await serverEvents.fetch({ since: baselineId });
  const redactEvents = newEvents.filter((e) => e.kind === "file-proxy.settings.redacted");
  assert.ok(
    redactEvents.length > 0,
    `expected redact events, got 0 (new events: ${JSON.stringify(newEvents.map((e) => e.kind))})`,
  );
  const snapshotEvents = redactEvents.filter(
    (e) => (e.detail as { site?: string } | undefined)?.site === "snapshot",
  );
  assert.ok(
    snapshotEvents.length > 0,
    `expected at least one redact event with site=snapshot, got sites: ${JSON.stringify(
      redactEvents.map((e) => (e.detail as { site?: string } | undefined)?.site),
    )}`,
  );

  // 守门：本 case 不应该出现 redact.bypassed event（bypass 只在 meta-redact-leak 触发）
  const bypassEvents = newEvents.filter((e) => e.kind === "file-proxy.settings.redact.bypassed");
  assert.equal(
    bypassEvents.length,
    0,
    `unexpected bypass events: ${JSON.stringify(bypassEvents)}`,
  );
...
test("F1-single-client-concurrent: 同 session 5 个并发 Bash → ack 配对", async () => {
  ...
  const cap = await mockAdmin.captured();
  assert.equal(cap.length, 2, `expected 2 turns, got ${cap.length}`);
  const toolResults = cap[1].toolResultsCurrentTurn;
  ...
  for (const t of tools) {
    ...
    // F1-CONCURRENT-N (N=0..4)：从 toolUseId 末尾 padded 数字还原 i
...
// ============================================================
// F3-multi-device: client-a / client-b 两容器并发，**两侧写入相同 relPath 但
// 不同 marker 内容**。主断言用 assertF3Isolation：通过单项 cache 查询验证
// A/B sha256 不同 + 各自 hash 匹配本端 marker。
// Codex 终审 §11.1 C3 要求："不能只验 deviceId 不同 + manifest 非空"。
// 选 .claude/CLAUDE.md（fresh device 走 SEED_WHITELIST 必扫到的路径）。
// ============================================================
test("F3-multi-device: client-a / client-b 并发 → cache 按内容 (sha256) 隔离", async () => {
  ...
  // 主断言：内容隔离。同 relPath 写不同 marker，断言两侧 sha256 不同 + 各自匹配本端 marker。
  // assertF3Isolation 也是 meta-deviceid-collision 的反向断言（强制同 deviceId 时
  // 应该 throw），保证主套件 case 与 meta-test 共用同一个不变量定义。
  ...
  // 旁证：两侧 manifest 都有数据（防 lookupEntry 命中但 manifest 整体损坏）
