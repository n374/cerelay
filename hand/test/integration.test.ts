...
test("CLI mode deferred MCP: prompt waits for slow MCP catalog ACK", async (t) => {
  const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "axon-hand-deferred-mcp-"));
  const brain = await startFakeBrain();
  registerBrainCleanup(t, brain);
  ...
  brain.ws.on("connection", (socket) => {
    ...
    socket.on("message", (raw) => {
      ...
      if (msg.type === "create_session") {
        socket.send(JSON.stringify({ type: "session_created", sessionId: "sess-deferred" }));
        return;
      }

      if (msg.type === "session_mcp_catalog") {
        timeline.push("mcp_catalog_received");
        // 模拟 MCP catalog 处理延迟（200ms），验证 prompt 会等待
        setTimeout(() => {
          timeline.push("mcp_catalog_applied_sent");
          socket.send(JSON.stringify({
  ...
  // session_created 后应立即显示 CLI 提示（不等 MCP）
  await waitFor(
    () => stdout.includes("你>") || child.exitCode !== null,
    3000,
    "等待 CLI 提示（延迟 MCP）"
  );
  assert.equal(stdout.includes("你>"), true, "MCP 延迟期间 CLI 提示应已显示");

  // 立即输入 — sendPrompt 应透明阻塞到 MCP 就绪后再发送
  child.stdin.write("hi\n");
  await waitFor(
    () => timeline.includes("prompt_received") || child.exitCode !== null,
    3000,
    "等待 prompt 到达 server"
  );
  ...
  // 关键断言：prompt 必须在 MCP catalog applied 之后到达 server
  const mcpAppliedIdx = timeline.indexOf("mcp_catalog_applied_sent");
  const promptIdx = timeline.indexOf("prompt_received");
  assert.notEqual(mcpAppliedIdx, -1, "MCP catalog 应已被处理");
  assert.notEqual(promptIdx, -1, "prompt 应已到达 server");
  assert.ok(
    mcpAppliedIdx < promptIdx,
    `prompt 应在 MCP catalog ACK 之后到达，实际顺序: ${timeline.join(" → ")}`
  );
});

test("CLI mode deferred MCP: session works even when MCP init fails", async (t) => {
  const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "axon-hand-mcp-fail-"));
  const brain = await startFakeBrain();
  registerBrainCleanup(t, brain);
  ...
  brain.ws.on("connection", (socket) => {
    ...
    socket.on("message", (raw) => {
      ...
      if (msg.type === "create_session") {
        socket.send(JSON.stringify({ type: "session_created", sessionId: "sess-mcp-fail" }));
        return;
      }

      if (msg.type === "session_mcp_catalog") {
        // 不回复 session_mcp_catalog_applied，改为直接关闭连接
        // 模拟 MCP catalog 处理失败
        setTimeout(() => socket.close(), 20);
        return;
      }
  ...
  // 即使 MCP 初始化失败，CLI 提示也应该出现
  await waitFor(
    () => stdout.includes("你>") || child.exitCode !== null,
    3000,
    "等待 CLI 提示（MCP 失败）"
  );
  assert.equal(stdout.includes("你>"), true, "MCP 失败后 CLI 提示应仍显示");
