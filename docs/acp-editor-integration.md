...
Axon Hand 支持以 ACP（Agent Communication Protocol）stdio 模式启动，让编辑器（Zed、VS Code 等）可以将 Axon 作为 Claude Code 使用。

ACP 使用 JSON-RPC 2.0 协议，通过标准输入/输出与编辑器通信。

Axon Hand supports running in ACP stdio mode, allowing editors (Zed, VS Code, etc.) to use Axon as Claude Code via the ACP protocol. ACP uses JSON-RPC 2.0 over stdin/stdout.

## 启动 ACP Server / Starting ACP Server

```bash
# 连接本地 Brain
axon-hand acp --server localhost:8765 --cwd /your/project

# 连接远程 Brain
axon-hand acp --server brain.example.com:8765 --cwd /your/project
```
...
**响应：**
```json
{"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2024-11-05","serverInfo":{"name":"axon-hand","version":"0.1.0"},"capabilities":{"streaming":true,"multiSession":true,"tools":["Read","Write","Edit","MultiEdit","Bash","Grep","Glob"]}}}
```
...
```bash
# 查看调试输出
axon-hand acp --server localhost:8765 2>acp.log
```
