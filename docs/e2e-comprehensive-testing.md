...
| 2026-05-03 | **F4 cross-cwd-fileproxy-isolation P2 PR1 闭环 ✅**(基础设施 / observability):Claude Subagent-Driven 13 task plan 的 T1-T7 + 多个 follow-up fix,共 18 commits(从 plan commit `0655a95` 到 PR1 闭环 `4292069`)。落地内容:(1) orchestrator detail interface 扩 `clientCwd / clientPath / contentSha256` + 2 个新 plan event interface;(2) 现有 helper 加 `clientCwd?` 过滤 + `assertNoReadServedForCwd` poll-and-collect negative-assert + `assertF4CrossCwdIsolation` 公共断言(失败时 dump 完整 probe 摘要);(3) `file-proxy.read.served` 4 出口加 cwd/path/sha256(thunk-based 生产零开销);(4) `client.requested` 3 处 + sideband 转录补 `clientCwd`(daemon 不补 clientPath,`fusePath` 替代);(5) daemon shadow sideband detail 加 `fusePath`;(6)(7) 新增 `config-preloader.plan` / `session.bootstrap.plan` admin event。e2e runner 顺手修 trap-based cleanup + stale GC(`test/cleanup-e2e-stale.sh`)。**验收**:e2e 29/29 + meta 3/3 + server unit 425/425 + client unit 135/135 + web unit 6/6 + typecheck 全过;Codex 终审 Conditional Approve(0 critical / 0 important / 2 minor 全修)。**下一步**:PR2 测试 case(T9-T13)落地。e2e coverage: 新增 `config-preloader.plan` / `session.bootstrap.plan` 协议事件维度 + 多个 fileProxy event detail 字段(clientCwd/clientPath/contentSha256/fusePath),已经在矩阵 §2.3 P2 backlog 跟踪 |
| 2026-05-05 | **F4 cross-cwd-fileproxy-isolation P2 PR2 闭环 ✅**(测试 case 落地):T9 fixture skeleton + T10 主 case + T11 meta failure case(toggle 反向回归)+ T12 入口扩。新增文件:`phase-p2.test.ts`(F4-cross-cwd-fileproxy-isolation 主 case,守 (a)/(b)/(c)/(d) 4 条不变量;6 类断言:stdout marker / read.served clientCwd / assertNoReadServedForCwd 双向 / configPreloader plan ancestorDirs 不串 / sessionBootstrap plan projectClaudeBindTarget / assertF4CrossCwdIsolation 综合)、`phase-p2-meta.test.ts`(注入 `injectCrossCwdRootCollision` toggle 让 sessionB project-claude root 错挂到 cwdA → assert.rejects 期望 throw,防 helper 自身退化)、`phase-p2-fixtures.ts`(共享 fixture 定义,避免 meta 与主 case test 注册串台)。新增 server 改动:`injectCrossCwdRootCollision` test-toggle(双重 gate);`SessionBootstrapPlanDetail.projectClaudeBindTarget` 字段语义统一为 FUSE source path(`FileProxyManager.roots["project-claude"]` 当前值,通过 `getProjectClaudeFuseSource()` getter)消除 server.ts 重复 toggle 判断;`assertF4CrossCwdIsolation` 加强加 read.served / client.requested clientPath 串台检查覆盖路由层错挂。`run-e2e-comprehensive.sh` 主套件加 phase-p2.test.ts 入口,`run-e2e-comprehensive-meta.sh` 链路加 phase-p2-meta.test.ts。**最终交付**:e2e **30/30** (P0 16 + P1-A 2 + P1-B 11 + P2 1) + meta **4/4** (P0 3 + P2 1) + server unit 425/425 + client unit 135/135 + web unit 6/6 + typecheck 全过。INF-12 cwd-ancestor-N root 注册仍待独立需求处理(spec §8 + §2.3 backlog 跟踪)。**下一步**:Codex 终审 PR2 整体(T13)。e2e coverage: 新增 cross-cwd 隔离边界守护维度 (a)/(b)/(c)/(d) + `injectCrossCwdRootCollision` test-toggle 维度 |
...
| **INF-10** ❌ Won't fix | (探索后假设不成立) | A5 加固 | **P1-B 收尾测试 PR 6 探索结论**：实测发现 CC SDK `--disallowedTools` 在 PreToolUse hook 之前 short-circuit，cerelay `buildShadowFallbackReason` 路径在 mock e2e 中不可达；对照断言 baseline 即失败——A5 主断言 OR 正则的 `mcp__cerelay__bash` 分支是死代码，主套件实际命中的是 CC SDK 兜底文案。INF-10 反向加固假设(cerelay 引导参与命中)不成立，相关 toggle / pty-session.ts deny 改动 / meta case 已撤销。生产中 cerelay 引导路径可能在真模型场景被触发，由 `server/test/e2e-real-claude-bash.test.ts` 守护 |
...
| 日期 | 事件 | 验证 |
|---|---|---|
| 2026-05-03 | F4-cross-cwd-fileproxy-isolation P2 PR1(基础设施 / observability)闭环,Codex 终审 Conditional Approve(0 critical / 0 important / 2 minor 全修) | `bash test/run-e2e-comprehensive.sh` 29/29 + `bash test/run-e2e-comprehensive-meta.sh` 3/3 + `cd server && npm test` 425/425 + typecheck 全过 |
| 2026-05-05 | F4-cross-cwd-fileproxy-isolation P2 PR2(测试 case 落地)闭环:phase-p2.test.ts 主 case + phase-p2-meta.test.ts meta failure 反向回归 + phase-p2-fixtures.ts 共享 fixture | `bash test/run-e2e-comprehensive.sh` **30/30**(P0 16 + P1-A 2 + P1-B 11 + P2 1)+ `bash test/run-e2e-comprehensive-meta.sh` **4/4**(P0 3 + P2 1)+ server unit 425/425 + client unit 135/135 + web unit 6/6 + typecheck 全过 |
| 2026-05-05 | **F4 cross-cwd P2 整体 Codex 终审通过 ✅**:Codex T13 终审 Conditional Approve(0 critical / 1 important / 0 minor)→ Important = (b) shared cache 命中污染 home-clientCwd 强断言缺失 → commit `4d99bd4` 修复(phase-p2 阶段三加 home-claude / home-claude-json read.served event 遍历 + clientCwd 严格对齐断言,覆盖 cache / snapshot-cache / snapshot-client 三种 servedFrom 路径;无 console.warn 输出证明 cache 真有命中,(b) 不变量 runtime cache hit 守护证据完整)。Spec §6 守护意图自查 7 类反模式全过(其中 (c) 诚实声明守计算计划层 / INF-12 跟踪)。**整 plan 13 task 全部闭环**(从 plan commit `0655a95` 到 PR2 闭环 + Codex 修复 commit `4d99bd4`,跨 PR1 18 commits + PR2 11 commits 共约 29 commits)。**下一步**:等 INF-12 cwd-ancestor-N root 注册产品缺口的独立需求落地,再升级 (c) 守护到"真实 ancestor FUSE read"层 |
| 2026-05-05 | INF-12-cwd-ancestor-root-registration 产品缺口仍登记到 §2.3 P2 backlog,等独立需求处理(F4 P2 不变量 (c) 真实 ancestor FUSE read 守护待该 INF 落地后再加固;当前 P2 case 通过 `configPreloaderEvents.plan` 守"计算计划不串台",诚实声明) | 见 §2.3 backlog INF-12 行 |
