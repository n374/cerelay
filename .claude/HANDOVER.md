...
Axon 是 Claude Code 的分体式架构：Hand（TypeScript，用户交互+工具执行）↔ Axon Server（TypeScript，基于 Claude Agent SDK 的 Brain）↔ claude CLI（LLM 推理）。整个项目为纯 TypeScript 实现，使用 npm workspaces 管理。
