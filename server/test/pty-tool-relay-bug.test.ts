...
test("嫌疑2: Passthrough 模式当 Client cwd 不存在时，runtime.cwd 不应回退到空目录", async (t) => {
  ...
  // Docker 容器内不存在宿主机路径
  const nonExistentClientCwd = "/Users/n374/Documents/Code/axon";
  assert.ok(
    !existsSync(nonExistentClientCwd) || process.platform === "darwin",
    "此测试需要 cwd 不存在于当前环境（Docker 容器内）"
  );

  // 在非 Darwin 环境下（模拟 Docker）验证 fallback 行为
  if (existsSync(nonExistentClientCwd)) {
    t.skip("当前环境是 macOS 宿主机，cwd 存在，无法复现 Docker 场景");
    return;
  }
